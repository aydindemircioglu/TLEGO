...
lsdir <- function(path, format = "basename", recursive = FALSE, all = FALSE, listFiles = FALSE) { 
  # list directories 
  ...
  # piece together system command 
	if (listFiles == FALSE) {
		execFind <- paste("find", path, argRecursive, "-mindepth 1", "-type d", "-print", " | sort",  sep = " ") 
	}  else {
		execFind <- paste("find", path, argRecursive, "-mindepth 1", "-print", " | sort",  sep = " ") 
	}
