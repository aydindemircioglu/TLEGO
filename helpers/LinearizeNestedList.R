...
getResultsFromRegistry = function (registryPath = NULL) {
	library(checkmate)
	...
	if (testFile (reducedDataFile) == TRUE) {
		...
		results = reduceResultsExperimentsParallel(registry, njobs = 10000, fun = function(job, res){ LinearizeNestedList (res, LinearizeDataFrames=TRUE) })
