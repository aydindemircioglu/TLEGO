    ...
    if(sm->sparse_kernel_type > 0) {
        ...
        if(strcmp(sparm->sparse_kernel_file,"")) {
            if(struct_verbosity > 0)
                printf("Reading basis functions for sparse kernel expansion from '%s'...",sparm->sparse_kernel_file);
            fflush(stdout);
        ...
        if(struct_verbosity > 0)
            printf("Replacing feature vectors...");
        fflush(stdout);
...
void         parse_struct_parameters(STRUCT_LEARN_PARM *sparm)
{
    ...
    for(i=0; (i<sparm->custom_argc) && ((sparm->custom_argv[i])[0] == '-'); i++) {
        ...
        case 'b':
            i++;
            sparm->bias=atof(sparm->custom_argv[i]);
            break;
        case 'p':
            i++;
            sparm->prec_rec_k_frac=atof(sparm->custom_argv[i]);
            break;
        case 'k':
            i++;
            sparm->sparse_kernel_size=atol(sparm->custom_argv[i]);
            break;
        case 't':
            i++;
            sparm->sparse_kernel_method=atol(sparm->custom_argv[i]);
            break;
        case 'i':
            i++;
            sparm->preimage_method=atol(sparm->custom_argv[i]);
            break;
        case 'r':
            i++;
            sparm->recompute_rset=atol(sparm->custom_argv[i]);
            break;
        case 's':
            i++;
            sparm->shrinking=atol(sparm->custom_argv[i]);
            break;
        case 'f':
            i++;
            strcpy(sparm->sparse_kernel_file,sparm->custom_argv[i]);
            break;
        default:
...
void         parse_struct_parameters_classify(STRUCT_LEARN_PARM *sparm)
{
    ...
    for(i=0; (i<sparm->custom_argc) && ((sparm->custom_argv[i])[0] == '-'); i++) {
        ...
        case 'd':
            i++;
            sparm->classify_dense=atof(sparm->custom_argv[i]);
            break;
        default:
            printf("\nUnrecognized option %s!\n\n",sparm->custom_argv[i]);
            exit(0);
