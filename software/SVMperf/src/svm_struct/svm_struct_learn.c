...
//	fullround=(activenum == n);
            ...
            } while(use_shrinking && (activenum > 0) && (reachedWallTime ==0)); /* when using shrinking,
							repeat until all examples
							produced no constraint at
							least once */

		} while((((totconstraints - old_totconstraints) > tolerance) || dont_stop) && (reachedWallTime ==0));

    } while (((epsilon > sparm->epsilon) 
		|| finalize_iteration(ceps,0,sample,sm,cset,alpha,sparm)) && (reachedWallTime ==0));
...
//                 double q = now();
// 
//                 char currentModelFile[255];
//                 char cFilename[28];
//                 strcpy(currentModelFile, modelFilePath);
//                 sprintf(cFilename, "%10.15f", q);
...
// //strcpy( dest, str );
// //strcat( dest, ".txt" );
// //                tmp << param->modelPath << "/" << std::setprecision(32) << q << ".bsgd.model";
...
// 
    ...
    } while(reachedWallTime == 0 && (cached_constraint || (ceps > sparm->epsilon) ||
    ...
    if(struct_verbosity>=1) {
        ...
        printf("Primal objective value with nC as loss regularization parameter: pval'=%.5f\n",
               0.5*modellength*modellength+sample.examples[0].x.totdoc*sparm->C*viol);
        printf("Violation: viol=%.5f\n",
               viol);
        printf("Sample size: n=%lu\n",
               sample.examples[0].x.totdoc);
        printf("Total number of constraints in final working set: %i (of %i)\n",(int)cset.m,(int)totconstraints);
        printf("Number of iterations: %d\n",numIt);
        printf("Number of calls to 'find_most_violated_constraint': %ld\n",argmax_count);
        printf("Number of SV: %ld \n",svmModel->sv_num-1);
