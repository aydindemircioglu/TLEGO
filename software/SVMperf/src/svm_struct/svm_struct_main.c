...
char trainfile[700];           /* file with training examples */
char modelfile[700];           /* file for resulting classifier */
			   ...
			   int *alg_type)
{
  ...
  learn_parm->wallTime = -1;
  learn_parm->startTime = rnow();
  learn_parm -> saveExponential = -1;
  learn_parm -> saveFactor = -1;
  ...
  for(i=1;(i<argc) && ((argv[i])[0] == '-');i++) {
    switch ((argv[i])[1]) 
      ...
      case 'X': i++; strcpy(modelPath,argv[i]); break;
      case 'A': i++; learn_parm -> saveExponential=atof(argv[i]); break;
      case 'F': i++; learn_parm -> saveFactor = atof(argv[i]); break;
	  case 'L': i++; learn_parm -> wallTime=atoi(argv[i]); break;
...
void print_help()
{
  ...
  printf("Supercharge Options:\n");
  printf("         -L          -> walltime : set maximum walltime in minutes\n");
  printf("         -A          -> saveExponential : set exponential of time interval in seconds to save intermediate model\n");
  printf("         -F          -> saveFactor : set linear factor of time interval in seconds to save intermediate model\n");
  printf("         -X          -> path to save walltime models\n");
