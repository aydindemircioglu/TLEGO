     ...
     /* retrain: selects training mode (1=regular / 2=holdout) */
{
  ...
  for(;retrain && (!terminate);iteration++) {
	  ...
	  // check time
	  if (unow() - learn_parm->startTime >  learn_parm->wallTime + learn_parm->timeusedForModelSaving) {
    ...
    if(verbosity>=2) {
      printf(
	"XIteration %ld: ",iteration); fflush(stdout);
    }
    else if(verbosity==1) {
      printf("."); fflush(stdout);
    }
     ...
     /* maxdiff: returns maximum violation of KT-conditions */
{
  ...
  compute_shared_slacks(docs,label,a,lin,c,active2dnum,learn_parm,
			slack,alphaslack);
                            ...
                            /* repeat this loop until we have convergence */
	if(verbosity>=1) {
		printf("away are %f", unow() - learn_parm->startTime );
...
// 	  // check time
    ...
    if(verbosity>=2) {
      printf(
	"YIteration %ld: ",iteration); fflush(stdout);
    }
    else if(verbosity==1) {
      printf("."); fflush(stdout);
    }
