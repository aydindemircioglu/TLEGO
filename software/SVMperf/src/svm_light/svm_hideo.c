...
int optimize_hildreth_despo(long,long,double,double,double,long,long,long,double,double *,
				...
				double *,double *,double *,long *,double *,double *, LEARN_PARM*);
int solve_dual(long,long,double,double,long,double *,double *,double *,
	       double *,double *,double *,double *,double *,double *,
		   double *,double *,double *,double *,long, LEARN_PARM*);
...
/* only semi-definite. */
{
  ...
  result=optimize_hildreth_despo(qp->opt_n,qp->opt_m,
				 ...
				 dual,nonoptimal,buffer,&progress, learn_parm);
  if(verbosity>=3) { 
...
int optimize_hildreth_despo(n,m,precision,epsilon_crit,epsilon_a,maxiter,goal,
				...
				primal,init,dual,lin_dependent,buffer,progress, learn_parm)
							 ...
							 
{
  ...
  if((!changed) || (n_indep>1)) { 
    result=solve_dual(n_indep,m,precision,epsilon_crit,maxiter,g_new,g0_new,
		      ...
		      dual,dual_old,temp,goal,learn_parm);
  }
  else {
...
int solve_dual(n,m,precision,epsilon_crit,maxiter,g,g0,ce,ce0,low,up,primal,
			   d,d0,ig,dual,dual_old,temp,goal,learn_parm)
  ...
  while((retrain) && (maxviol > 0) && (iter < (scalemaxiter*maxfaktor))) {
    ...
    while((maxviol > precision) && (iter < (scalemaxiter*maxfaktor))) {
	  ...
	  if (iter % (10000000) == 0) {
		  // check time
		  if (verbosity > 3) {
			printf("away are %f", snow() - learn_parm->startTime );
			printf(" -- waiting for that to become larger than %f\n", learn_parm->wallTime + learn_parm->timeusedForModelSaving); 
		  }
		  if (snow() - learn_parm->startTime >  learn_parm->wallTime + learn_parm->timeusedForModelSaving) {
			  printf("FFFReached walltime, quitting..\n");
			  fflush(stdout);
			  terminate = 1;
			  break;
		  }
