...
MATRIX *incomplete_cholesky(DOC **x,long n,long *rank,double epsilon,
							KERNEL_PARM *kparm, long **index, LEARN_PARM *lparm)
...
{
	// yes, make it walltimed. very funny. but true.

	// anytime hack
	// FIXME: make it 'more' beautiful, haha.
	double walltime = lparm->wallTime;
	double starttime = lparm->startTime;
  ...
  for(i=0;i<*rank;i++) {
	  ...
	  if ((walltime != -1) && (funnynow() - starttime >= (walltime) * 1))
	  {
		  // force quit
  ...
  (*index)=pindex;
  *rank = newrank;
  return(realloc_matrix(G,newrank,newrank));
}
