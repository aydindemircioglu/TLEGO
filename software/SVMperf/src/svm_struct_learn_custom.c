...
// dirty hack for anytime algorithm
double xnow()
{
	struct timeval t;
	gettimeofday(&t, NULL);
        ...
        STRUCTMODEL *sm)
{
	...
	// turn off saving at all
	double saveExponential = lparm -> saveExponential;
	double saveFactor = lparm -> saveFactor;
	
	timeusedForModelSaving = 0;
	double modelsWritten = 0;
	double nextSaveTime = saveFactor * (modelsWritten+1) + floor (pow (saveExponential, 1));

	double algorithmStartTime = xnow();
	
	wallTime = lparm->wallTime;
	startTime = lparm->startTime;
		
	printf("ID: SVMperf loop 3\n");
	printf("Save Exponential: [%f]\n", saveExponential);
	printf("Save Factor: [%f]\n", saveFactor);
	printf("Walltime: [%f]\n", wallTime);
	printf("Algorithm Time at start: [%f]\n", algorithmStartTime);
	printf("Algorithm Time at start: [%f]\n", startTime);
	printf("Next save time: [%f]\n", nextSaveTime);
	...
	do { /* iteratively find and add constraints to working set */
		...
		if (numIt > 1) {
			...
			if (xnow() - startTime >  wallTime + timeusedForModelSaving) {
        ...
        if((ceps > sparm->epsilon) || (rset_maxsize>sm->reducedset_size) ||
            ...
            if(recompute_kernel_matrix) {
				...
				MATRIX *tmpM = init_kernel_matrix_rset(&rcset,kparm,sm);
				
				// CHECK FOR RUNTIME. these are (now) global variables
				if (xnow() - startTime >  wallTime + timeusedForModelSaving) {
					printf("Reached walltime, quitting..\n");
					fflush(stdout);
					free_matrix(tmpM);
			...
			// bad hack for getting walltime in underlying svmlight
			
			lparm->startTime = startTime;
			lparm->wallTime = wallTime;
			lparm->timeusedForModelSaving = timeusedForModelSaving;
			
// 			printf("starttime: [%f]\n", kparm->startTime);
// 			printf("walltime: [%f]\n", kparm->wallTime);
// 			printf("timeusedForModelSaving: [%f]\n", kparm->timeusedForModelSaving);
// 			printf("diff now: [%f]\n", kparm->startTime - xnow());
// 
		...
		//printf ("%d -- %d ",(currentTime - algorithmStartTime), (nextSaveTime + timeusedForModelSaving ));
		if ((saveExponential >= 0.0) && (currentTime - algorithmStartTime >= nextSaveTime + timeusedForModelSaving )) {
			modelsWritten++;
			nextSaveTime = saveFactor * (modelsWritten+1)  + pow (saveExponential, modelsWritten + 1);
			double before = xnow();

			printf("&");
			fflush(stdout);
								
			char currentModelFile[777];
			char cFilename[28];
			
			if (strlen(modelPath) < 2) {
				strcpy(currentModelFile, modelFile);
				strcat(currentModelFile, "_");
			} else {
				strcpy(currentModelFile, modelPath);
				strcat(currentModelFile, "/");
			}
			sprintf(cFilename, "%d", (int)(floor(before - algorithmStartTime)));
			strcat(currentModelFile, cFilename);
			if (strlen(modelPath) < 2) {
			} else {
				strcat(currentModelFile, ".svmperf.model");
			}
			printf("Saving to %s\n", currentModelFile);

			// writing model is not that easy, damn!  XXX
			{
				/**** build normal SVM model from exansion ****/
				MODEL m;
				m.at_upper_bound = svmModel -> at_upper_bound;
				m.b = svmModel -> b;
				m.totwords = svmModel -> totwords;
				m.totdoc = svmModel -> totdoc;
				m.kernel_parm = svmModel -> kernel_parm;
				...
				for(j=1; j<m.sv_num; j++) {
					m.alpha[j]=svmModel->lin_weights[j];
					m.supvec[j]=create_example(j,0,1,1,copy_svector(sm->reducedset[j-1]));
				}
				m.lin_weights=NULL;
				m.index=NULL;

				MODEL *tmpModel = sm->svm_model;
				sm->svm_model=&m;
				double *tmpw = sm->w;
				sm->w=NULL;
				write_struct_model(currentModelFile,sm,sparm);
				sm->w = tmpw;
				sm->svm_model = tmpModel;
			}
...
/* selects between different methods for computing the best preimage approximation for psi-psihat */
{
    ...
    find_preimage_count++;
	if(struct_verbosity >= 2) {
		printf("%d", sparm->preimage_method);
		fflush(stdout);
	}
...
/* start is used as the starting point */
{
    ...
    for(i=0,delta=0; (delta<0.99999) && (i<maxiter) && (fabs(sum)>10E-90); i++) {
		...
		// we need to check if we hit some walltime XXX
