...
extern double saveExponential;
extern double saveFactor;
...
//			info(".");
		...
		if ((saveExponential >= 0.0) && (currentTime - algorithmStartTime >= nextSaveTime + timeusedForModelSaving )) {
			...
			double before = now();
				...
				if (modelPath.length() < 2) {
					tmp << modelFileName << "_" << std::setprecision(32) << floor( now() - algorithmStartTime - timeusedForModelSaving);
				} else {
					tmp << modelPath << "/" << std::setprecision(32) << floor( now() - algorithmStartTime - timeusedForModelSaving) << ".libsvm.model";
				}
	...
	if(iter >= max_iter)
	{
		...
		printf("\nWARNING: reaching max number of iterations or the specified walltime.\n");
	}
