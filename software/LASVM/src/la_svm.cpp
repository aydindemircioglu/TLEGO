...
void exit_with_help() {
    fprintf (stdout,
             ...
             "-A saveExponential : set exponential of time interval in seconds to save intermediate model (default -1)\n"
             "-X modelPath: path to save walltime models (default -1)\n"
             "-F saveFactor : set linear factor of time interval in seconds to save intermediate model (default -1)\n"
            );
    exit (1);
}
...
void parse_command_line (int argc, char **argv, char *input_file_name, char *model_file_name) {
    ...
    for (i = 1; i < argc; i++) {
        ...
        switch (argv[i - 1][1]) {
            ...
            case 'A':
                saveExponential = atof (argv[i]);
                break;

            case 'X':
                strcpy (modelPath, argv[i]);
                break;

            case 'F':
                saveFactor  = atof (argv[i]);
                break;
            ...
            if (deltamax <= 1000) { // potentially multiple calls to reprocess..
                ...
                while (lasvm_get_delta (sv) > deltamax && deltamax < 1000) {
					...
					if ( (saveExponential >= 0.0) && (currentTime - algorithmStartTime >= nextSaveTime + timeusedForModelSaving)) {
						printf ("Saving model at %f..\n", currentTime - algorithmStartTime);
						modelsWritten++;
						...
						// save model
						{
							...
							if (strlen (modelPath) < 2) {
								sprintf (tmp, "%s_%d", model_file_name, (int) floor (before - algorithmStartTime - timeusedForModelSaving));
							} else {
								sprintf (tmp, "%s/%d.lasvm.model", modelPath, (int) floor (before - algorithmStartTime - timeusedForModelSaving));
							}
