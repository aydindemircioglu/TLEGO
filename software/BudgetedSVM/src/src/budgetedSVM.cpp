...
void printUsagePrompt(bool trainingPhase, parameters *param)
{
	...
	if (trainingPhase)
	{
		...
		svmPrintString(text);
        sprintf(text, " l - walltime : set maximum walltime in minutes (%d)\n\n", (*param).wallTime);
        svmPrintString(text);	
        sprintf(text, " a - saveExponential : set exponential of time interval in seconds to save intermediate model (%d)\n\n", (*param).saveExponential);
        svmPrintString(text);
        sprintf(text, " f - saveFactor : set linear factor of time interval in seconds to save intermediate model (%d)\n\n", (*param).saveFactor);
        svmPrintString(text);
        sprintf(text, " x - path to save walltime models (%s)\n\n", (*param).modelPath.c_str());
        svmPrintString(text);
	}
	else
	{
...
void parseInputPrompt(int argc, char **argv, bool trainingPhase, char *inputFile, char *modelFile, char *outputFile, parameters *param)
{
	...
	for (i = 1; i < argc; i++)
	{
        ...
        svalue.push_back(argv[i]);
	}

	// we now start measuring
	(*param).startTime = xnow();
	...
	if (trainingPhase)
	{
		...
		for (unsigned int i = 0; i < option.size(); i++)
		{
			...
			{
                case 'a':
                    (*param).saveExponential = (double) value[i];
                    break;
                    
                case 'f':
                    (*param).saveFactor = (double) value[i];
                    break;
                ...
                case 'l':
                    (*param).wallTime = (unsigned int) value[i];
                    break;
