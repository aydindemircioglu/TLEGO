	...
	public:		
		...
		*/
		...
		/*! \fn void extendDimensionalityOfModel(unsigned int newDim, parameters* param)
			\brief Extends the dimensionality of each linear hyperplane in the AMM model.
		...
		void extendDimensionalityOfModel(unsigned int newDim, parameters* param)
		{
			...
			for (unsigned int i = 0; i < (*modelMM).size(); i++)
				for (unsigned int j = 0; j < (*modelMM)[i].size(); j++)
					(*modelMM)[i][j]->extendDimensionality(newDim, param);
		};
...
/*! \fn float predictAMM(budgetedData *testData, parameters *param, budgetedModelAMM *model, vector <int> *labels = NULL, vector <float> *scores = NULL)
