...
void trainBSGD(budgetedData *trainData, parameters *param, budgetedModelBSGD *model)
{
	...
	double wallTime = param -> wallTime;
	double saveExponential = param->saveExponential;
	double saveFactor = param -> saveFactor;

	double startTime = param -> startTime;
	...
	double algorithmStartTime = now();
	int walltimeReached = 0;
	for (unsigned int epoch = 0; epoch < (*param).NUM_EPOCHS && walltimeReached == 0; epoch++)
	{
		//Calculate
		...
		while (stillChunksLeft)
		{
			...
			start = clock();
			for (ot = 0; ot < N && walltimeReached == 0; ot++)
			{
				t = tv[ot];
				numIter++;
				...
				// check if need to write our model
				...
				if ((saveExponential >= 0.0) && (currentTime - algorithmStartTime >= nextSaveTime + timeusedForModelSaving )) {
					...
					if ((param -> modelPath).length() > 2) {
						tmp << param->modelPath << "/" << std::setprecision(32) << floor( now() - algorithmStartTime - timeusedForModelSaving) << ".bsgd.model";
					} else {
						tmp << param->modelFile << "_" << std::setprecision(32) << floor( now() - algorithmStartTime - timeusedForModelSaving);
					}
					model->saveToTextFile(tmp.str().c_str(), &(trainData->yLabels), param);
				...
				if (now() - startTime >  wallTime + timeusedForModelSaving) {
					std::cout << "Hit the walltime, exiting now..\n";
					walltimeReached = 1;
					break;
				}
