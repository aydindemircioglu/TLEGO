...
void Cache::swap_index(int i, int j)
{
	...
	svmhelper::swap(head[i].data,head[j].data);
	svmhelper::swap(head[i].len,head[j].len);
	if(head[i].len) lru_insert(&head[i]);
	if(head[j].len) lru_insert(&head[j]);

	if(i>j) svmhelper::swap(i,j);
	for(head_t *h = lru_head.next; h!=&lru_head; h=h->next)
	{
		if(h->len > i)
		{
			if(h->len > j)
				svmhelper::swap(h->data[i],h->data[j]);
			else
			{
				// give up
				lru_delete(h);
...
{
	svmhelper::swap(x[i],x[j]);
	if(x_square) svmhelper::swap(x_square[i],x_square[j]);
}
...
{
	Q->swap_index(i,j);
	svmhelper::swap(y[i],y[j]);
	svmhelper::swap(G[i],G[j]);
	svmhelper::swap(alpha_status[i],alpha_status[j]);
	svmhelper::swap(alpha[i],alpha[j]);
	svmhelper::swap(b[i],b[j]);
	svmhelper::swap(active_set[i],active_set[j]);
	svmhelper::swap(G_bar[i],G_bar[j]);
}
...
public:
	...
	void swap_index(int i, int j) const
	{
		svmhelper::swap(sign[i],sign[j]);
		svmhelper::swap(index[i],index[j]);
		svmhelper::swap(QD[i],QD[j]);
	}
	...
	double Cp, double Cn)
{
	...
	switch(param->svm_type)
	{
		...
		case NU_SVR:
			...
			break;
		case BVM:        
			solve_bvm(prob,param,alpha,&si,Cp,Cn);
			break;
		case CVDD:
			solve_cvdd(prob,param,alpha,&si);
			break;
        case CVM:
			solve_cvm(prob,param,alpha,&si,Cp,Cn);
            break;
		case CVR:
		case CVM_LS:
			solve_cvr(prob,param,alpha,&si);
            break;
	}

	if (param->svm_type == CVDD)
		info("radius=%f, ||c||^2=%f\n",si.rho,si.margin);
	else
		info("obj=%g, bias=%f, margin=%f\n",si.obj,si.rho,si.margin);
...
svm_model *svm_train(const svm_problem *prob, svm_parameter *param)
{
	   ...
	   param->svm_type == CVR)
	{
		...
		model->rho[0]   = f.rho;
		model->cNorm    = Malloc(double,1);
		model->cNorm[0] = f.cNorm;
	...
	else
	{
		...
		for(i=0;i<nr_class;i++)
		{
			for(int j=i+1;j<nr_class;j++)
			{
				...
				// add infos for intermediate model saving
				param->nr_classes = nr_class;
				param->label = label;
				param->count = count;
				param->start = start;
				param->prob = &sub_prob;
	    ...
	    param->svm_type == NU_SVC) && nr_fold < l)
	{
		...
		for (c=0; c<nr_class; c++) 
			for(i=0;i<count[c];i++)
			{
				...
				svmhelper::swap(index[start[c]+j],index[start[c]+i]);
			}
		for(i=0;i<nr_fold;i++)
		{
			fold_count[i] = 0;
...
{
	if(model->param.svm_type == CVDD)
	{
		double *sv_coef = model->sv_coef[0];
		double sum = 0;
		for(int i=0;i<model->l;i++)
			sum += sv_coef[i] * Kernel::k_function(x,model->SV[i],model->param);
		sum = model->rho[0] - (model->cNorm[0]-2.0*sum + Kernel::k_function(x,x,model->param));
		*dec_values = sum;
	}
	else if(model->param.svm_type == ONE_CLASS ||
	   model->param.svm_type == EPSILON_SVR ||
	   model->param.svm_type == NU_SVR ||
	   model->param.svm_type == CVR )
	{
		double *sv_coef = model->sv_coef[0];
		double sum = 0;
		for(int i=0;i<model->l;i++)
...
double svm_predict(const svm_model *model, const svm_node *x)
{
	if(model->param.svm_type == ONE_CLASS ||
	   model->param.svm_type == EPSILON_SVR ||
	   model->param.svm_type == NU_SVR ||
	   model->param.svm_type == CVDD ||
	   model->param.svm_type == CVR )
	{
		double res;
		svm_predict_values(model, x, &res);
		
		if(model->param.svm_type == ONE_CLASS || model->param.svm_type == CVDD)
			return (res>0)?1:-1;		
		else
			return res;
	}
...
{
	"c_svc","nu_svc","one_class","epsilon_svr","nu_svr","cvdd","cvc","cvc_ls","cvr","bvm","bvm_2",NULL
};
...
svm_model *svm_load_model(const char *model_file_name)
{
	...
	while(1)
	{
		...
		else if (strcmp(cmd,"cNorm")==0)
		{
			int n = model->nr_class * (model->nr_class-1)/2;
			model->cNorm = Malloc(double,n);
			for(int i=0;i<n;i++)
				fscanf(fp,"%lf",&model->cNorm[i]);
		}
...
const char *svm_check_parameter(const svm_problem *prob, const svm_parameter *param)
{
	...
	if((kernel_type != RBF && kernel_type != EXP && kernel_type != NORMAL_POLY && kernel_type != INV_SQDIST && kernel_type != INV_DIST) 
	   && (svm_type == BVM ))
	...
	if((param->svm_type != CVDD && param->svm_type != CVM && param->svm_type != CVM_LS && param->svm_type != CVR 
		&& param->svm_type != BVM ) && param->eps <= 0)
		return "eps <= 0";
	...
	if(svm_type == CVR || svm_type == CVM_LS)
		if(param->mu < 0.0)
			return "mu < 0";
...
void displayInfoAboutModel(const struct svm_model* model)
{
	if(model->param.svm_type >= BVM)
	{
		...
		for(i=0;i<nr_class;i++)
		{
			for(int j=i+1;j<nr_class;j++)
			{
				...
				for(k1=0;k1<ci;k1++)
				{
					for(k2=0;k2<ci;k2++)
					{
						rho += coef1[si+k1]*coef1[si+k2]*(1.0+Kernel::k_function(model->SV[si+k1],model->SV[si+k2],model->param));
					}

					for(k2=0;k2<cj;k2++)
					{
						rho += coef1[si+k1]*coef2[sj+k2]*(1.0+Kernel::k_function(model->SV[si+k1],model->SV[sj+k2],model->param));
					}

					rho += coef1[si+k1]*coef1[si+k1]/1.0e0;
					bias += coef1[si+k1];
				}
				...
				for(k1=0;k1<cj;k1++)
				{
					for(k2=0;k2<ci;k2++)
					{
						rho += coef2[sj+k1]*coef1[si+k2]*(1.0+Kernel::k_function(model->SV[sj+k1],model->SV[si+k2],model->param));
					}

					for(k2=0;k2<cj;k2++)
					{
						rho += coef2[sj+k1]*coef2[sj+k2]*(1.0+Kernel::k_function(model->SV[sj+k1],model->SV[sj+k2],model->param));
					}

					rho += coef2[sj+k1]*coef2[sj+k1]/1.0e0;
					bias += coef2[sj+k1];
				}
		...
		// dump the database to std. output (SVs), labels are based on the alphas of (0,1) classes train
/*
		for(i=0;i<model->l;i++)
		{
			info("%d",  ((start[0]<=i) && (i<start[0]+model->nSV[0])) ? 1 : 0);
			for(const svm_node *sv = model->SV[i];sv->index!=-1;sv++)
			{
				info(" %d:%lg", sv->index, sv->value);
			}
			info("\n");
		}
*/
