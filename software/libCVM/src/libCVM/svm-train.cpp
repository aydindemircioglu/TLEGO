	...
	for(i=1;i<argc;i++)
	{
		...
		switch(argv[i-1][1])
		{
			...
			case 'a':
				...
				break;
			case 'L':
				param.wallTime = atof(argv[i]);
				printf("walltime set to: %f seconds.\n", param.wallTime);
				break;
			case 'A':
				param.saveExponential = atof(argv[i]);
				printf("saveExponential set to: %f.\n", param.saveExponential);
				break;
			case 'X':
				param.modelPath = (argv[i]);
				printf("modelPath set to: %s.\n", param.modelPath);
				break;
			case 'F':
				param.saveFactor = atof(argv[i]);
				printf("saveFactor set to: %f.\n", param.saveFactor);
				break;
			default:
